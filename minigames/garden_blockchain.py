...
class GardenBlockchain:
    """Garden Direct Private Key Disbursement using GAMES_KEY"""
    ...
    def __init__(self):
        ...
        # Private key for Garden disbursements (same as other minigames)
        self.games_key = os.getenv('GAMES_KEY')

        # Initialize Web3
        self.w3 = Web3(Web3.HTTPProvider(self.celo_rpc_url))

        if self.w3.is_connected():
            logger.info("✅ Connected to Celo network for Garden")
        else:
            logger.error("❌ Failed to connect to Celo network")

        logger.info("🌾 Garden Private Key Service initialized")
    ...
    async def disburse_garden_reward(self, wallet_address: str, amount: float) -> dict:
        """
        Disburse Garden rewards via direct private key transfer using GAMES_KEY
        ...
        try:
            ...
            logger.info(f"🌾 Garden reward disbursement: {amount} G$ to {masked_wallet}")

            if not self.games_key:
                logger.error("❌ GAMES_KEY not configured for Garden disbursement")
                return {"success": False, "error": "Games key not configured"}
            ...
            try:
                if self.games_key.startswith('0x'):
                    games_account = Account.from_key(self.games_key)
                else:
                    games_account = Account.from_key('0x' + self.games_key)
                logger.info(f"🔑 Using GAMES_KEY account: {self.mask_wallet_address(games_account.address)}")
            except Exception as key_error:
                logger.error(f"❌ Failed to load GAMES_KEY: {key_error}")
            ...
            try:
                nonce = self.w3.eth.get_transaction_count(games_account.address)
            ...
            try:
                transfer_txn = contract.functions.transfer(
                    ...
                    'gas': 250000,  # 250k gas limit for Garden withdrawals
                    'gasPrice': gas_price,
                    'nonce': nonce,
                    'from': games_account.address
                })
            ...
            try:
                signed_txn = self.w3.eth.account.sign_transaction(transfer_txn, self.games_key)
            ...
            try:
                ...
                logger.info(f"🔗 Garden transaction sent: {tx_hash_hex}")
            ...
            if receipt.status == 1:
                logger.info(f"✅ Garden reward successfully disbursed to {masked_wallet}. TX: {tx_hash_hex}")
                return {
                    "success": True,
                    "tx_hash": tx_hash_hex,
                    "amount": amount,
                ...
                logger.error(f"❌ Garden transaction failed on-chain. TX: {tx_hash_hex}")
                ...
                try:
                    ...
                    if "insufficient funds" in error_msg or "insufficient balance" in error_msg:
                        logger.error(f"❌ Insufficient funds in GAMES_KEY wallet")
                        logger.error(f"❌ GAMES_KEY wallet needs refunding for Garden rewards")
                        return {
                            "success": False,
                            "error": "Insufficient funds in treasury",
                            "message": "The Garden treasury needs to be refunded. Please contact support.",
                            "tx_hash": tx_hash_hex,
                            "explorer_url": f"https://explorer.celo.org/mainnet/tx/{tx_hash_hex}"
                        }
            ...
            logger.error(f"❌ Garden reward disbursement error: {e}", exc_info=True)
            return {"success": False, "error": str(e)}

    def disburse_garden_reward_sync(self, wallet_address: str, amount: float) -> dict:
        """Synchronous wrapper for disburse_garden_reward to avoid event loop issues"""
        import asyncio
        ...
        try:
            ...
            try:
                ...
                return asyncio.run(self.disburse_garden_reward(wallet_address, amount))
        except Exception as e:
    ...
    def _run_in_new_loop(self, wallet_address: str, amount: float) -> dict:
        ...
        try:
            return loop.run_until_complete(self.disburse_garden_reward(wallet_address, amount))
        finally:
            loop.close()


# Global instance
garden_blockchain_service = GardenBlockchain()
